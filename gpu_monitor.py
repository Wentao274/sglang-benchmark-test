...
class GPUMonitor:
    ...
    def check_gpu_tool(self):
        ...
        try:
            result = subprocess.run(
                ["mx-smi", "--version"], capture_output=True, text=True
            )
            if result.returncode == 0:
                self.monitor_cmd = "mx-smi"
                return True
        except:
            pass
        return False
    ...
    def _monitor_loop(self):
        while self.running:
            try:
                ...
                elif self.monitor_cmd == "mx-smi":
                    csv_file = self.log_file.replace(
                        ".log", f"_{timestamp.replace(':', '').replace('-', '')}.csv"
                    )
                    result = subprocess.run(
                        [
                            "mx-smi",
                    ...
                    if result.returncode == 0 and os.path.exists(csv_file):
                        try:
                            with open(csv_file, "r") as csvf:
                                output_lines = csvf.read().strip().split("\n")
                            data_written = False
                            with open(self.log_file, "a") as f:
                                for line in output_lines:
                                    line = line.strip()
                                    ...
                                    if len(parts) >= 7:
                                        try:
                                            ...
                                            mem_used = parts[2] if parts[2] else "0"
                                            mem_total = parts[3] if parts[3] else "0"
                                            gpu_util = parts[4] if parts[4] else "0"
                                            mem_util = parts[5] if parts[5] else "0"
                                            temp = parts[6] if parts[6] else "0"
                                            f.write(
                                                f"{timestamp},{gpu_idx},{gpu_name},{mem_used},{mem_total},{gpu_util},{mem_util},{temp}\n"
                                            )
                                            data_written = True
                            ...
                            if not data_written and output_lines:
                                print(
                                    f"Warning: No data parsed from mx-smi output. First few lines: {output_lines[:5]}"
                                )
                        finally:

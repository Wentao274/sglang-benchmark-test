...
def run_benchmark(chip_name, base_config, model_config, test_suites, run_id):
    base_url = base_config.get("base_url", "http://127.0.0.1:30000")
    ...
    for test_suite in test_suites:
        ...
        random_input_output_len = test_params.get(
            "random-input-output-len", [[1024, 256]]
        )
...
def generate_reports(chip_name, model_name, test_suites, concurrency=None):
    for test_suite in test_suites:
        ...
        run_ids = get_available_run_ids(chip_name, model_name, test_suite)
        ...
        if len(run_ids) == 1:
            ...
            run_single_report(chip_name, model_name, test_suite, run_id, concurrency)
        else:
            print(f"Multiple run-ids found, generating comparison report...")
            run_runid_comparison(
                chip_name, model_name, test_suite, run_ids, concurrency
            )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Run SGLang benchmark and generate reports"
    )
    parser.add_argument(
        "--chip",
        type=str,
        required=True,
        help="Chip name to test (e.g., inspur_MetaX_C550, nvidia_h100)",
    )
    parser.add_argument(
        "--model",
        type=str,
        default=None,
        help="Model name to test. If not specified, uses the first model in config.",
    )
    parser.add_argument(
        "--test-suite",
        type=str,
    ...
    chip_key = chip_key_map.get(chip_name, chip_name)

    if chip_key not in models:
        print(
            f"Error: Chip '{chip_key}' not found in config. Available chips: {', '.join(models.keys())}"
        )
        return

    available_models = models[chip_key]
    ...
    if args.model:
        ...
        if not selected_model:
            print(
                f"Error: Model '{args.model}' not found for chip '{chip_key}'. Available models:"
            )
            for m in available_models:
    ...
    for model_config in model_configs:
        ...
        print(f"\n{'#' * 60}")
        print(f"Processing chip: {chip_key}, model: {model_name}")
        print(f"{'#' * 60}")

        if not args.only_report:
            if not args.skip_benchmark:
                run_benchmark(
                    chip_key, base_config, model_config, test_suites_to_run, run_id
                )
            else:
                print("Skipping benchmark execution (--skip-benchmark specified)")
                print("Loading config for report generation...")

        print(f"\nFinished benchmark for chip: {chip_key}, model: {model_name}")

        print(f"\n{'#' * 60}")
        print("Generating reports...")
        print(f"{'#' * 60}")

        generate_reports(chip_key, model_name, test_suites_to_run, args.concurrency)

        print(f"\n{'#' * 60}")
        print(f"All reports generated for chip: {chip_key}, model: {model_name}")
        print(f"{'#' * 60}")

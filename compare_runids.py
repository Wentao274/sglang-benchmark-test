...
def generate_comparison_charts(
    ...
    for run_id in run_ids:
        all_run_ids_data[run_id] = {
            ...
            "e2e": get_values(run_id, "P99 E2E Latency (ms)"),
        }

    metrics = [
        ("Request Throughput (req/s)", "req_tp", axes[0, 0]),
        ("Total Token Throughput (tok/s)", "total_tp", axes[0, 1]),
        ("TTFT P99 (ms)", "ttft", axes[0, 2]),
        ("TPOT P99 (ms)", "tpot", axes[1, 0]),
        ("ITL P99 (ms)", "itl", axes[1, 1]),
        ("E2E Latency P99 (ms)", "e2e", axes[1, 2]),
    ]
...
"""
    ...
    sglang_info = load_sglang_config_by_model(chip_name, actual_model_name)
    sglang_table_rows = []
    for param, val in sglang_info.items():
        if param == "remarks":
            continue
        if val is None:
            val = "N/A"
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Chip name (e.g., inspur_MetaX_C550, nvidia_h100)",
    )
